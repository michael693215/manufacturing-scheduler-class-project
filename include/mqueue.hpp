...
const int MINZERO = 0;
const int PREORDER = 0; // preorder heapified array
const int INORDER = 1;  // inorder heapified array
const int MINONE = 1;
...
public:
    ...
    // The following function writes the heap to a json file using preorder
    // traversal
    void writeHeap(json&);        
...
private:
     ...
     ******************************************/
    Node* buildNode(Order);                     // for creating new nodes
    Node* copyHeap(Node*);                      // returns a deep copy of a heap
    Node* merge(Node*, Node*);                  // merges two nodes
    int calculateNPL(Node*);                    // update the npl
    void swapChildren(Node*, Node*, Node*);     // swap children
    Node* highestPriority(Node*, Node*);        // returns node with higher priority (if a node is a root, it should be left param)
    void saveHeap(Node**, int&, Node*);         // puts heap inside an array
    Node* unlinkHeap(Node*);                    // sets all children in heap to nullptr
    void printPreorder(Node*) const;            // helper for printOrderQueue
    void writeHeapPreorder(Node*, json&); // helper for writing the heap into a json file
    void writeHeapInorder(Node*, json&);   // helper for writing the heap into a json file
};
